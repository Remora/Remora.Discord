    ...
    public class DiscordRestGuildAPITests
    {
        ...
        public class CreateGuildIntegrationAsync : RestAPITestBase<IDiscordRestGuildAPI>
        {
            ...
            public async Task PerformsRequestCorrectly()
            {
                var guildId = new Snowflake(0);
                var type = "www";
                var id = new Snowflake(1);

                var api = CreateAPI
                (
                    b => b
                        .Expect(HttpMethod.Post, $"{Constants.BaseURL}guilds/{guildId}/integrations")
                        .WithJson
                        (
                            j => j.IsObject
                            (
                                o => o
                                    .WithProperty("type", p => p.Is(type))
                                    .WithProperty("id", p => p.Is(id.ToString()))
                            )
                        )
                        .Respond(HttpStatusCode.NoContent)
                );

                var result = await api.CreateGuildIntegrationAsync
                (
                    guildId,
                    type,
                    id
        ...
        public class ModifyGuildIntegrationAsync : RestAPITestBase<IDiscordRestGuildAPI>
        {
            ...
            {
                var guildId = new Snowflake(0);
                var integrationId = new Snowflake(1);
                var expireBehaviour = IntegrationExpireBehaviour.Kick;
                var expireGracePeriod = 2;
                var enableEmoticons = true;

                var api = CreateAPI
                (
                    b => b
                        .Expect(HttpMethod.Patch, $"{Constants.BaseURL}guilds/{guildId}/integrations/{integrationId}")
                        .WithJson
                        (
                            j => j.IsObject
                            (
                                o => o
                                    .WithProperty("expire_behaviour", p => p.Is((int)expireBehaviour))
                                    .WithProperty("expire_grace_period", p => p.Is(expireGracePeriod))
                                    .WithProperty("enable_emoticons", p => p.Is(enableEmoticons))
                            )
                        )
                        .Respond(HttpStatusCode.NoContent)
                );

                var result = await api.ModifyGuildIntegrationAsync
                (
                    guildId,
                    integrationId,
                    expireBehaviour,
        ...
        public class DeleteGuildIntegrationAsync : RestAPITestBase<IDiscordRestGuildAPI>
        {
            ...
            public async Task PerformsRequestCorrectly()
            {
                ...
                var result = await api.DeleteGuildIntegrationAsync
                (
                    guildId,
                    integrationId
                );
        ...
        public class SyncGuildIntegrationAsync : RestAPITestBase<IDiscordRestGuildAPI>
        {
            ...
            public async Task PerformsRequestCorrectly()
            {
                ...
                (
                    ...
                    (
                        HttpMethod.Post,
                        $"{Constants.BaseURL}guilds/{guildId}/integrations/{integrationId}/sync"
                    )
                    .Respond(HttpStatusCode.NoContent)
                );

                var result = await api.SyncGuildIntegrationAsync
                (
                    guildId,
                    integrationId
                );
